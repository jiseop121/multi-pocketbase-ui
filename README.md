...
`pbviewer`는 여러 PocketBase 인스턴스를 별칭 기반으로 관리하고, 컬렉션/레코드를 읽기 전용으로 조회하는 로컬 CLI 도구입니다.
...
```bash
pbviewer -c "db add --alias local --url http://127.0.0.1:8090"
pbviewer -c "db list"
```
...
```bash
pbviewer -c "superuser add --db local --alias root --email root@example.com --password pass123456"
pbviewer -c "superuser list --db local"
```

### 4) API 조회

```bash
pbviewer -c "api collections --db local --superuser root"
...
```bash
pbviewer -c "api records --db local --superuser root --collection posts --format csv --out ./posts.csv"
```
