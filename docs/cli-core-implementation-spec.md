...
Track 1 범위에서 `pbmulti`의 성공 경로(REPL / one-shot / script)를 한 번에 읽히게 구현하고, 실패를 일관된 오류 형식과 종료 코드(`0/1/2/3`)로 고정한다.
...
#### B. One-shot (`pbmulti -c "<command>"`)
1. `-c` 텍스트를 단일 명령으로 파싱한다.
2. dispatcher로 1회 실행한다.
3. 결과 출력 후 종료한다.

#### C. Script (`pbmulti <script-file>`)
1. UTF-8 파일을 읽고 줄 단위로 순회한다.
2. 빈 줄/`#` 주석 줄은 건너뛴다.
3. 명령 실행 중 첫 실패에서 즉시 중단한다(fail-fast).
4. 실패 시 `Error: Script failed at line <N>: <message>`를 출력하고 종료한다.

#### D. UI 예약 플래그 (`pbmulti -ui`) - Track 1
1. `-ui`를 예약 플래그로 인식한다.
...
- `cmd/pbmulti/main.go`는 `app.Run(...)`만 호출한다.
...
| `runScript` | `context, path` | `error` | 파일 라인 실행 + fail-fast |
...
Tests: 모드 결정/충돌, script fail-fast(line 번호), 읽기 전용 규칙, 포맷-옵션 조합, 종료 코드/출력 채널 분리를 계약 테스트로 검증한다.
